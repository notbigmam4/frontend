...
const Førerkort = ({
  ...
  interactive: boolean,
  data: { name: string | undefined, birthday: string | undefined, img: string | undefined } | undefined
}) => {
    ...
    <>
      ...
      >
        ...
        >
          ...
          <h1 className="text-[24px] text-[#444f55] mt-2">
            {data ? data?.name?.toUpperCase() : globalcontext?.user?.name.toUpperCase()}
          </h1>

          <h5 className="text-[16px] text-[#444f55] mt-1 font-light">
            {data ? data.birthday : globalcontext?.user?.birthday} 81271
          </h5>
          ...
          <div className="flex gap-3 w-full h-fit ml-8 mt-10 z-10">
            <InfoButton className="border-l-[5px] flex flex-col items-start justify-start p-3 w-[44.5%]">
              <div className="text-[42px] leading-none font-bold text-[#444f55] mb-6 text-left">
                B
              </div>
              <img
                className="h-[42px] w-auto object-contain self-start"
                alt="car"
                src={caricon}
              />
            </InfoButton>
